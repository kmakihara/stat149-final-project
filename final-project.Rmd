...
```{r}
stepwise_deviance_selector = function (mytrain, selected_predictors, log=FALSE) 
{
...
anova(model08.1, model08.2, test="Chisq")
...
### Selected Model 'model14.1' Prediction
```{r}
...
stepwise_deviance_selector(within(mytrain.na, rm("density_urban")), c("combined_ethnicity_4way", "density_rural", "sex", "interest_in_religion", "liberal_donor", "guns_1", "density_suburban", "conservative_donor", "cnty_pct_evangelical"), log = TRUE)
model26 = glm(formula = suppdem ~ combined_ethnicity_4way + density_rural + sex + interest_in_religion + liberal_donor + guns_1 + density_suburban + conservative_donor + cnty_pct_evangelical + contbrel_1, family = binomial, data = within(mytrain.loginc, rm("density_urban")))
...
model27 = glm(formula = suppdem ~ combined_ethnicity_4way + density_rural + sex + interest_in_religion + liberal_donor + guns_1 + density_suburban + conservative_donor + cnty_pct_evangelical + contbrel_1 + single, family = binomial, data = within(mytrain.loginc, rm("density_urban")))
stepwise_deviance_selector(within(mytrain.loginc, rm("density_urban")), c("combined_ethnicity_4way", "density_rural", "sex", "interest_in_religion", "liberal_donor", "guns_1", "density_suburban", "conservative_donor", "cnty_pct_evangelical", "contbrel_1", "single"))
# ERROR ! FUCK ! GONNA STOP
```
